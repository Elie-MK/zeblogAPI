...
export class LikesService {
    constructor(@InjectRepository(Likes) private readonly likesRepository: Repository<Likes>) {}

    async createLike(idArticles: number, idUser: number, LikeDto:likeDto) {
        const newLike = this.likesRepository.create({idArticles, idUser, likeStatus:LikeDto.likeStatus});
        await this.likesRepository.save(newLike);
        return newLike;
    }

    modifyLike(idArticles: number, idUser: number, LikeDto:likeDto) {
        return this.likesRepository.update({idArticles, idUser}, {likeStatus:LikeDto.likeStatus});
    }
